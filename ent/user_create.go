...
func (uc *UserCreate) SetInterests(i []int) *UserCreate {
	uc.mutation.SetInterests(i)
	return uc
}
...
func (uc *UserCreate) AddLikedPostIDs(ids ...int) *UserCreate {
...
func (uc *UserCreate) AddBookmarkedPostIDs(ids ...int) *UserCreate {
	uc.mutation.AddBookmarkedPostIDs(ids...)
	return uc
}

// AddBookmarkedPosts adds the "bookmarked_posts" edges to the Post entity.
func (uc *UserCreate) AddBookmarkedPosts(p ...*Post) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddBookmarkedPostIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.LikedPostsIDs(); len(nodes) > 0 {
		...
		createE := &LikeCreate{config: uc.config, mutation: newLikeMutation(uc.config, OpCreate)}
		createE.defaults()
		_, specE := createE.createSpec()
		edge.Target.Fields = specE.Fields
		_spec.Edges = append(_spec.Edges, edge)
	}
	if nodes := uc.mutation.BookmarkedPostsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
		...
		createE := &BookmarkCreate{config: uc.config, mutation: newBookmarkMutation(uc.config, OpCreate)}
		createE.defaults()
		_, specE := createE.createSpec()
		edge.Target.Fields = specE.Fields
		_spec.Edges = append(_spec.Edges, edge)
