...
func (cu *CategoryUpdate) AddPost(p ...*Post) *CategoryUpdate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (cu *CategoryUpdate) ClearPost() *CategoryUpdate {
...
func (cu *CategoryUpdate) RemovePost(p ...*Post) *CategoryUpdate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (cu *CategoryUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if cu.mutation.PostCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
			Table:   category.PostTable,
			Columns: []string{category.PostColumn},
			Bidi:    false,
...
// AddPost adds the "post" edges to the Post entity.
func (cuo *CategoryUpdateOne) AddPost(p ...*Post) *CategoryUpdateOne {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
// ClearPost clears all "post" edges to the Post entity.
func (cuo *CategoryUpdateOne) ClearPost() *CategoryUpdateOne {
...
// RemovePost removes "post" edges to Post entities.
func (cuo *CategoryUpdateOne) RemovePost(p ...*Post) *CategoryUpdateOne {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
