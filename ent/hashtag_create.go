...
func (hc *HashtagCreate) SetTitle(s string) *HashtagCreate {
	hc.mutation.SetTitle(s)
	return hc
}

// AddPostIDs adds the "posts" edge to the Post entity by IDs.
func (hc *HashtagCreate) AddPostIDs(ids ...int) *HashtagCreate {
	hc.mutation.AddPostIDs(ids...)
	return hc
}

// AddPosts adds the "posts" edges to the Post entity.
func (hc *HashtagCreate) AddPosts(p ...*Post) *HashtagCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (hc *HashtagCreate) check() error {
	if _, ok := hc.mutation.Title(); !ok {
		return &ValidationError{Name: "title", err: errors.New(`ent: missing required field "Hashtag.title"`)}
	}
