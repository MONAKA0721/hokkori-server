...
func (cq *CategoryQuery) QueryPost() *PostQuery {
...
func (cq *CategoryQuery) WithPost(opts ...func(*PostQuery)) *CategoryQuery {
	query := &PostQuery{config: cq.config}
...
func (cq *CategoryQuery) loadPost(ctx context.Context, query *PostQuery, nodes []*Category, init func(*Category), assign func(*Category, *Post)) error {
	...
	query.Where(predicate.Post(func(s *sql.Selector) {
		s.Where(sql.InValues(category.PostColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.post_category
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_category" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
